...
%% Color Plot
plot_tor(1).n = 20; % Number of slats on cross section, 0 for no plot
...
plot_tor(1).def = 1;
plot_tor(1).fig = 5; %figure number
plot_tor(1).u_comp = 3; % U_component = 1 for x, 2 for y, 3 for z, 4 for magnitude
plot_tor(1).auto_scale = 0; % auto color scale switch 
plot_tor(1).caxis = [0 25]; % Min and max displacement scale
plot_tor(1).color = 'k'; % color  of fill or wireframe
plot_tor(1).view = 7; % TF = 1 for Bottom View, 
    % TF = 2 for Top View
    % TF = 3 for Right View
    % TF = 4 for Left View
    % TF = 5 for Back View
...
plot_tor(3).view = 6; % TF = 1 +Y, Z vert
                      % TF = 2 -Y, Z vert
                      % TF = 3 -X, Y vert
                      % TF = 4 +X, Y vert
                      % TF = 5 +Z, Y vert
                      % TF = 6 -Z, Y vert
                      % TF = 7 ISO
plot_tor(3).el_type = [2]; % plotting elements other than 5 
