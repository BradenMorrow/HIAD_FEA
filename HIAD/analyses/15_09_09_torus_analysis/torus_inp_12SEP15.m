...
% Second step
% proc_tor_load_test_15APR15 % NAMESPACE POLUTION
F_ind = J(n + 1:end - 3);
...
P_strap(:,:,2) = -P*[cos(theta(J(n + 1:n + nP))) sin(theta(J(n + 1:n + nP))) zeros(size(sin(theta(J(n + 1:n + nP))),1),4)];



% Torus test
tor_F2 = zeros(size(tor_bound,1),size(P_strap,3));
...
type = 5*ones(size(tor_connect,1),1);
...
p = 18; % Internal pressure
r = 13.6/2; % Shell (minor) radius
beta = 71; % Braid angle
props12 = [14444.41201	0.001	11.67629614	0]; % Shell properties % [15655.8378114503 0.001 11.3599117025832 0]; % 
