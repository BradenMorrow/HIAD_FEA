...
eps_cord  = eps_cord_old;
f_cord = f_cord_old;
DELTA_eps_cord_old = eps_cord*0;
DELTA_f_cord = 0;

eps_cord_plus = eps_cord_old;
% f_cord_plus = f_cord_old;
DELTA_eps_cord_plus_old = eps_cord*0;
DELTA_f_cord_plus = 0;
...
%     [~,d1i,d1j] = get_eps(eps0 + eps,y_bar,r,phi_i,phi_j,alpha,kappa2);
    ...
    delta_eps_cord = DELTA_eps_cord - DELTA_eps_cord_old;
    DELTA_eps_cord_old = DELTA_eps_cord;
%     eps_cord = eps_cord_old + delta_eps;
%     delta_eps_cord = eps_cord - eps_cord_old;
    eps_cord = eps_cord + delta_eps_cord;
    ...
    %         force = interp1(axial(:,2),axial(:,1),eps_cord(j,i),interp_meth);
    %         force_up = interp1(axial(:,2),axial(:,1),eps_cord(j,i) + tol,interp_meth);
            force = interp1(axial(:,2),axial(:,1),eps_cord(i,j),interp_meth);
            force_up = interp1(axial(:,2),axial(:,1),eps_cord(i,j) + tol,interp_meth);
            k = (force_up - force)/tol;
            k_cord(j,i) = k;
         end
    end
    ...
    DELTA_f_cord = DELTA_f_cord + delta_f_cord;
    f_cord = f_cord + delta_f_cord;
...
%     f = sum(f_cord)' + 2*pi*r*t*EL*y_bar.*kappa2 + EL*A*eps;
    ...
    delta_eps_cord_plus = DELTA_eps_cord_plus - DELTA_eps_cord_plus_old;
    DELTA_eps_cord_plus_old = DELTA_eps_cord_plus;
    eps_cord_plus = eps_cord_plus + delta_eps_cord_plus;
...
%     eps_cord_plus = delta_eps_cord_plus + eps_cord_old;
    ...
    %         force = interp1(axial(:,2),axial(:,1),eps_cord(j,i),interp_meth);
    %         force_up = interp1(axial(:,2),axial(:,1),eps_cord(j,i) + tol,interp_meth);
            force = interp1(axial(:,2),axial(:,1),eps_cord_plus(i,j),interp_meth);
            force_up = interp1(axial(:,2),axial(:,1),eps_cord_plus(i,j) + tol,interp_meth);
            k = (force_up - force)/tol;
            k_cord_plus(j,i) = k;
         end
    end
    ...
    DELTA_f_cord_plus = DELTA_f_cord_plus + delta_f_cord_plus;
