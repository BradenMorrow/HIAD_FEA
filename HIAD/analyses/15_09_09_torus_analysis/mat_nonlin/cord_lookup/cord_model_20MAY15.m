function [d] = cord_model_20MAY15(eps,F,load_point,unload_point,d_LOAD,d_UNLOAD)
%CORD_MODEL_06MAY15
...
unload_point2 = unload_point;
if sum(eps <= load_point(:,2)) > 0
    unload_point2 = [unload_point2; load_point(load_point(:,2) >= eps,:)];
end

while sum(unload_point2(2:end,1) - unload_point2(1:end - 1,1) < 0) < size(unload_point2,1) - 2
    i_mark1 = [unload_point2(2:end,1) - unload_point2(1:end - 1,1) < 0; true];
    i_mark1(1) = true;
    unload_point2 = unload_point2(i_mark1(1:end),:);
end

while sum(unload_point2(2:end,2) - unload_point2(1:end - 1,2) < 0) < size(unload_point2,1) - 2
    i_mark1 = [unload_point2(2:end,2) - unload_point2(1:end - 1,2) < 0; true];
    i_mark1(1) = true;
    unload_point2 = unload_point2(i_mark1(1:end),:);
end
% for i = 2:size(load_point,1)
%     if eps < load_point(i,2)
%         if i < size(load_point,1)
%             if eps > load_point(i + 1,2)
%                 unload_point2 = [unload_point2; load_point(i,:)];
%             end
%         end
%     end
% end

d_load = zeros(size(unload_point2,1),2);
d_load(1,:) = [F eps];
ind = 2;
for i = size(unload_point2,1):-1:2
    d_load(ind,:) = unload_point2(i,:);
    ind = ind + 1;
end
