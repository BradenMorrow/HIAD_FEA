...
plot_tor(1).LS = 0; % load step plotted
...
plot_tor(1).frac = 4; % 1/frac is fraction of data used
plot_tor(1).tor_num = 6; % Number of tori present
...
plot_tor(2).tor_num = 6; % Number of tori present

%% Undeformed with Fill
plot_tor(3).n = 100; % Number of slats on cross section, 0 for no plot
...
plot_tor(4).tor_num = 6; % Number of tori present

%% Undeformed Wireframe
plot_tor(5).n = 20; % Number of slats on cross section, 0 for no plot
plot_tor(5).LS = 1; % load step plotted
plot_tor(5).PF = 0; % plot forces?
plot_tor(5).plot_type = 5; % 1 = DCP, 2 = DF, 3 = UF, 4 = DWF, 5 = UWF 
plot_tor(5).def = 0;
plot_tor(5).fig = 3; % figure number
...
plot_tor(5).frac = 3; % 1/frac is fraction of data used
plot_tor(5).tor_num = 9; % Number of tori present
...
else post_proc(FEM,plot_tor)
end


FEM.PLOT.els = [0 0 0 1]';
FEM.PLOT.fig = 3;
