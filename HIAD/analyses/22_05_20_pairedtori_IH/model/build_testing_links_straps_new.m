...
for i = 1:size(tori_nodes)
    tan1 = tangentPoint([test_rad, test_z],...
        [sqrt(FEM.MODEL.nodes(tori_nodes(i),1)^2+...
        FEM.MODEL.nodes(tori_nodes(i),2)^2),...
        FEM.MODEL.nodes(tori_nodes(i),3)],r(1),0);

    tan2 = tangentPoint([test_rad, test_z],...
        [sqrt(FEM.MODEL.nodes(length(tori_theta)+tori_nodes(i),1)^2+...
        FEM.MODEL.nodes(length(tori_theta)+tori_nodes(i),2)^2),...
        FEM.MODEL.nodes(length(tori_theta)+tori_nodes(i),3)],r(2),1);
    ...
    z3 = test_z;
    nodes3(i,:) = [x3 y3 z3];
    point1 = [sqrt(FEM.MODEL.nodes(tori_nodes(i),1)^2+...
        FEM.MODEL.nodes(tori_nodes(i),2)^2),...
        FEM.MODEL.nodes(tori_nodes(i),3)];

    point2 = [sqrt(FEM.MODEL.nodes(length(tori_theta)+tori_nodes(i),1)^2+...
        FEM.MODEL.nodes(length(tori_theta)+tori_nodes(i),2)^2), ...
        FEM.MODEL.nodes(length(tori_theta)+tori_nodes(i),3)];

    tangent  = circle_tan([[point1(1), point1(2)], [point2(1), ...
        point2(2)]], [r(1) r(2)], 1); % Find tangent line beetween tori
...
nodes = [nodes1; nodes2; nodes3; nodes4; nodes5; nodes6; nodes7; nodes8;...
    nodes10; nodes11; nodes12; nodes13; nodes14; nodes15];
...
connect_l2 = [size(tori_theta,1) + tori_nodes(1:size(theta,1)) ...
    size(FEM.MODEL.nodes,1) + size(theta,1) + (1:(size(theta,1)))'];

connect_l3 = [tori_nodes(1:size(theta,1)) size(FEM.MODEL.nodes,1) + ...
    size(theta,1)*3 + (1:(size(theta,1)))'];

connect_l4 = [size(tori_theta,1) + tori_nodes(1:size(theta,1))...
    size(FEM.MODEL.nodes,1) + size(theta,1)*4 + (1:(size(theta,1)))'];
...
% Straps
connect_s1 = [size(FEM.MODEL.nodes,1) + (1:(size(theta,1)))' ...
    size(FEM.MODEL.nodes,1) + size(theta,1)*2 + (1:(size(theta,1)))'];

connect_s2 = [size(FEM.MODEL.nodes,1) + size(theta,1) + ...
    (1:(size(theta,1)))' size(FEM.MODEL.nodes,1) + size(theta,1)*2 + ...
    (1:(size(theta,1)))'];

connect_s3 = [size(FEM.MODEL.nodes,1) + size(theta,1)*3 + ...
    (1:(size(theta,1)))' size(FEM.MODEL.nodes,1) + size(theta,1)*4 +...
    (1:(size(theta,1)))'];
...
connect_nl1 = [tori_nodes(1:size(theta,1)) size(FEM.MODEL.nodes,1)+...
    size(theta,1)*8 + (1:(size(theta,1)))'];

connect_nl2 = [tori_nodes(1:size(theta,1)) size(FEM.MODEL.nodes,1)+...
    size(theta,1)*9 + (1:(size(theta,1)))'];

connect_nl3 = [size(tori_theta,1)+tori_nodes(1:size(theta,1)) ...
    size(FEM.MODEL.nodes,1)+ size(theta,1)*10 + (1:(size(theta,1)))'];

connect_nl4 = [size(tori_theta,1)+tori_nodes(1:size(theta,1)) ...
    size(FEM.MODEL.nodes,1)+ size(theta,1)*11 + (1:(size(theta,1)))'];

connect_nl5 = [size(tori_theta,1)+tori_nodes(1:size(theta,1)) ...
    size(FEM.MODEL.nodes,1)+ size(theta,1)*12 + (1:(size(theta,1)))'];

connect_nl6 = [size(tori_theta,1)+tori_nodes(1:size(theta,1)) ...
    size(FEM.MODEL.nodes,1)+ size(theta,1)*13 + (1:(size(theta,1)))'];

connect_nl = [connect_nl1; connect_nl2; connect_nl3; connect_nl4;...
    connect_nl5; connect_nl6;];

con_newlink = [connect_nl 2*ones(size(connect_nl,1),1)];

% New Straps
connect_ns1 = [size(FEM.MODEL.nodes,1) + (1:(size(theta,1)))' ...
    size(FEM.MODEL.nodes,1)+size(theta,1)*8 + (1:(size(theta,1)))'];

connect_ns2 = [size(FEM.MODEL.nodes,1)+size(theta,1)*8 + ...
    (1:(size(theta,1)))' size(FEM.MODEL.nodes,1)+size(theta,1)*9 +...
    (1:(size(theta,1)))'];

connect_ns3 = [size(FEM.MODEL.nodes,1)+size(theta,1)*9 + ...
    (1:(size(theta,1)))' size(FEM.MODEL.nodes,1) + size(theta,1)*3 +...
    (1:(size(theta,1)))'];

connect_ns4 = [size(FEM.MODEL.nodes,1) + size(theta,1)*4 +...
    (1:(size(theta,1)))' size(FEM.MODEL.nodes,1)+size(theta,1)*10 + ...
    (1:(size(theta,1)))'];

connect_ns5 = [size(FEM.MODEL.nodes,1)+size(theta,1)*10 + ...
    (1:(size(theta,1)))' size(FEM.MODEL.nodes,1)+size(theta,1)*11 + ...
    (1:(size(theta,1)))'];

connect_ns6 = [size(FEM.MODEL.nodes,1)+size(theta,1)*11 +...
    (1:(size(theta,1)))' size(FEM.MODEL.nodes,1)+size(theta,1)*12 +...
    (1:(size(theta,1)))'];

connect_ns7 = [size(FEM.MODEL.nodes,1)+size(theta,1)*12 +...
    (1:(size(theta,1)))' size(FEM.MODEL.nodes,1)+size(theta,1)*13 +...
    (1:(size(theta,1)))'];

connect_ns8 = [size(FEM.MODEL.nodes,1)+size(theta,1)*13 + ...
    (1:(size(theta,1)))' size(FEM.MODEL.nodes,1) + size(theta,1) +...
    (1:(size(theta,1)))'];

connect_ns = [connect_ns1; connect_ns2; connect_ns3; connect_ns4; 
    connect_ns5; connect_ns6; connect_ns7; connect_ns8];

con_newstrap = [connect_ns 1*ones(size(connect_ns,1),1)];

cable_post = [size(FEM.MODEL.connect,1)+size(con_link,1)+...
    size(strap_link,1)+size(bound_link,1)+1, size(FEM.MODEL.connect,1)+...
    size(con_link,1)+size(strap_link,1)+size(bound_link,1)+...
    size(cable_link,1)];
