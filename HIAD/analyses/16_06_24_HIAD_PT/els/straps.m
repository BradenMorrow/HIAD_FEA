function [MODEL,EL,strap_ind1] = straps(theta,I_theta,str,strap_offset)
...
[loop_f,loop_k] = strap_response(114000,100,50,2,1);
[rad_f,rad_k] = strap_response(99751,300,100,2,1);
[chev_f,chev_k] = strap_response(114000,114000,10,1,1);

if strap_offset
    loop_f.breaks = loop_f.breaks - loop_f.breaks(3);
    loop_f.coefs(:,4) = loop_f.coefs(:,4) - 50;
    loop_k.breaks = loop_k.breaks - loop_k.breaks(3);
    ...
    rad_f.breaks = rad_f.breaks - rad_f.breaks(3);
    rad_f.coefs(:,4) = rad_f.coefs(:,4) - 100;
    rad_k.breaks = rad_k.breaks - rad_k.breaks(3);
...
%% ORIENTATION
orientation = zeros(size(connect,1),3);
orientation(:,3) = 200;
