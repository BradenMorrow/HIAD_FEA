function [MODEL,EL] = tor5(theta,C,r,cone_ang,torID)
...
x5 = C(torID,1) + r(torID)*sind(cone_ang);
y5 = C(torID,2) - r(torID)*cosd(cone_ang);
x6 = C(torID + 1,1) + r(torID + 1)*sind(cone_ang);
y6 = C(torID + 1,2) - r(torID + 1)*cosd(cone_ang);
trib = sqrt((C(torID,1) - C(torID - 1,1))^2 + (C(torID,2) - C(torID - 1,2))^2)/2 + sqrt((x6 - x5)^2 - (y6 - y5)^2)*r(torID)/(r(torID) + r(torID + 1));
