...
pur_offset = 0; % Offset of purlins
W = 5.7; % Bent width
H_walls = 2.69; % Wall height
B_sp = 2.5; % Bent spacing
R_th = 13.324; % Roof angle
N_bents = 5; % Number of bents
N_pur = 5; % Number of purlins per slope
...
% Columns
Ncol = 10;
n_post_L = [linspace(N_gross(1,1),N_gross(2,1),Ncol)' zeros(Ncol,1) linspace(N_gross(1,3),N_gross(2,3),Ncol)'];
c_post_L = [(1:Ncol - 1)' (2:Ncol)'];

n_post_R = [linspace(N_gross(3,1),N_gross(4,1),Ncol)' zeros(Ncol,1) linspace(N_gross(3,3),N_gross(4,3),Ncol)'];
c_post_R = c_post_L;

% Roof
a = linspace(N_gross(5,1),N_gross(6,1),N_pur)';
a1 = linspace(N_gross(5,3),N_gross(6,3),N_pur)';
b = linspace(N_gross(7,1),N_gross(6,1),N_pur)';
b1 = linspace(N_gross(7,3),N_gross(6,3),N_pur)';
...
% Purlin ties
c = a - pur_offset*sind(R_th);
c1 = a1 + pur_offset*cosd(R_th);
d = b + pur_offset*sind(R_th);
d1 = b1 + pur_offset*cosd(R_th);

n_roof_tie_L = [a zeros(size(a)) a1
    c zeros(size(c)) c1];
c_roof_tie_L = [(1:N_pur)' (N_pur + 1:N_pur*2)'];

n_roof_tie_R = [b zeros(size(b)) b1
    d zeros(size(d)) d1];
c_roof_tie_R = c_roof_tie_L;
...
nodes0 = [n_post_L
    ...
    n_tie];
connect = [c_post_L
    c_post_R + size(n_post_L,1)
    c_roof_L + size(n_post_L,1) + size(n_post_R,1)
    c_roof_tie_L + size(n_post_L,1) + size(n_post_R,1) + size(n_roof_L,1)
    c_roof_R + size(n_post_L,1) + size(n_post_R,1) + size(n_roof_L,1) + size(n_roof_tie_L,1)
    c_roof_tie_R + size(n_post_L,1) + size(n_post_R,1) + size(n_roof_L,1) + size(n_roof_tie_L,1) + size(n_roof_R,1)
    c_tie + size(n_post_L,1) + size(n_post_R,1) + size(n_roof_L,1) + size(n_roof_tie_L,1) + size(n_roof_R,1) + size(n_roof_tie_R,1)];
...
for i = 2:N_bents
    ...
    connect = [connect; connect1 + n_node];
...
[nodes,iu,inu] = unique(nodes0,'stable','rows'); % Remove redundant nodes
con = [inu(connect(:,1)) inu(connect(:,2))]; % Update connectivities
...
hold on
axis equal
plot3(nodes(:,1),nodes(:,2),nodes(:,3),'ko','markersize',2)
