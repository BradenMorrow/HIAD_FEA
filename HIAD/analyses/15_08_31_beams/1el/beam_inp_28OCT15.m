...
% beam_F([6 - nDOF Di]) = [-react react]'; % Load (lb)
...
% alpha = [90 210 330]'; % [60 180 300]'; % alpha_i; % Location of cords on shell cross section % 
if nDOF == 0
    alpha = [60 180 300]';
elseif nDOF == 1;
    alpha = [90 210 330]';
...
for i = 1:N - 1
    ...
    EL(i).el_in0.flex.D = zeros(3,Nint); % Section forces
    EL(i).el_in0.flex.Q = zeros(5,1); % Element forces
    EL(i).el_in0.flex.f = zeros(3,3,Nint); % Section compliance matrices
    EL(i).el_in0.flex.d = zeros(3,Nint); % Section deformations
