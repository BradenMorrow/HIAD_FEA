...
n_cable = 100; % Number of radial cables
el_int = 1; % Number of elements between cable nodes
...
tor.R = 59.1890;
tor.nodes = tor.R*[cos(theta) sin(theta) zeros(size(theta))];
Rave = tor.R*ones(size(theta));
...
tor.ELong = EL; % 85; % 0; % 80; % 25; % 100; % Longitudinal shell modulus (lb/in)
tor.GLH = GLH; % 5316; % 2379; In-plane shear modulus (lb/in)
% % % cable_pp = tor_test.pp; % Cable load response


for j = 1:n_cable*2
    [f,k] = lin_strap_response(1140000,300,.001);
...
% % %     plot_tor_shape_post(tor_test,FEM_out0,theta_support)
    ...
    % Load cable ends
    Fmag = 11000/n_cable/2; % 5230/n_cable/2; % 84; % Cable end force (preload)
    [FEM1.MODEL.F_pt] = cable_F(ind,theta,I_theta_cable,Fmag);
    
    % Controls
    [FEM1.ANALYSIS] = FE_controls1_60;
    FE_plot(FEM1)
    ...
    % Displace cable ends
    Umag = 0.6; % *.2378; % Cable end displacment
    ...
    plot([-.6 0]*25.4,[1 1]*2*pi*p_i*pi*r_i^2*(1 - 2*cotd(57)^2)*4.448222/1000,'k--')
    xlim([-15 0])
