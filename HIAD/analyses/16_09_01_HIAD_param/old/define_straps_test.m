function [straps] = define_straps_test(C,r,alpha_cone)
...
straps(S).num_straps = 148; % Number of straps around HIAD
...
w = 2; % Strap width (in)
straps(S).geom = [t*w w*t^3/12 w^3*t/12 0 .312*w*t^3]; % Element geometry, local z perpendicular to global theta [A Izz Iyy ky J]
straps(S).theta0 = 360/148/2; % Location of first strap
straps(S).num_straps = 148; % Number of straps around HIAD
straps(S).theta_sweep = 0; % Skew strap, angle swept by strap, measured from connectivity 1 to 2, CCW+
...
straps(S).theta0 = 0; % Location of first strap
straps(S).num_straps = 148; % Number of straps around HIAD
straps(S).theta_sweep = 360/148/4; % Skew strap, angle swept by strap, measured from connectivity 1 to 2, CCW+
