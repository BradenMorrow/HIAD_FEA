function [ANALYSIS] = FE_controls_beam_const_M
% Analysis controls for FE analysis

% Solver and analysis
ANALYSIS.d_inc = 20; % Starting number of loading increments
ANALYSIS.control = 2; % 1 - Displacement control solver;
                      % 2 - Newton-Raphson solver;
                      % 3 - Arc-length solver;
                      % 4 - Enforce boundary conditions
ANALYSIS.NR_iter = 100; % Newton-Raphson, update stiffness matrix every 'NR_iter' iterations
ANALYSIS.max_inc_count = 200; % Maximum number of increments
ANALYSIS.min_inc_size = 1e-10; %Minimum increment size to abort program

ANALYSIS.max_iter = 5; % Maximum number of iterations per increment
ANALYSIS.tol = 1e-7; % Solver tolerance, (normalized to force vector)
ANALYSIS.Dmax =5.5; % Absolute maximum displacement
