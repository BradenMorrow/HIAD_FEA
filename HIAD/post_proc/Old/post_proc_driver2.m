%% Color Plot
plot_tor(1).n = 4; % Number of slats on cross section, 0 for no plot
plot_tor(1).LS = size(FEM_out.OUT.Uinc,2); % load step plotted
plot_tor(1).PF = 0; % plot forces?
plot_tor(1).plot_type = 1; % 1 = DCP, 2 = DF, 3 = UF, 4 = DWF, 5 = UWF 
plot_tor(1).def = 1;
plot_tor(1).fig = 6; %figure number
plot_tor(1).u_comp = 4; % U_component = 1 for x, 2 for y, 3 for z, 4 for magnitude
plot_tor(1).auto_scale = 1; % auto color scale switch 
plot_tor(1).caxis = [.0013 .003]; % Min and max displacement scale
...
plot_tor(2).LS = size(FEM_out.OUT.Uinc,2); % load step plotted
...
plot_tor(2).view = 6; % TF = 1 +Y, Z vert
                      % TF = 2 -Y, Z vert
                      % TF = 3 -X, Y vert
                      % TF = 4 +X, Y vert
                      % TF = 5 +Z, Y vert
                      % TF = 6 -Z, Y vert
                      % TF = 7 ISO
plot_tor(2).el_type = [2 4]; % plotting elements other than 5 
plot_tor(2).scale = 1; % scale of deformation
plot_tor(2).triad = 1; % triad switch
...
plot_tor(3).fig = 1; %figure number
...
plot_tor(3).color = 'g'; % color  of fill or wireframe
plot_tor(3).view = 1; % TF = 1 +Y, Z vert
                      % TF = 2 -Y, Z vert
                      % TF = 3 -X, Y vert
                      % TF = 4 +X, Y vert
                      % TF = 5 +Z, Y vert
...
plot_tor(4).view = 6; % TF = 1 +Y, Z vert
                      % TF = 2 -Y, Z vert
                      % TF = 3 -X, Y vert
                      % TF = 4 +X, Y vert
                      % TF = 5 +Z, Y vert
...
plot_tor(5).view = 6; % TF = 1 +Y, Z vert
                      % TF = 2 -Y, Z vert
                      % TF = 3 -X, Y vert
                      % TF = 4 +X, Y vert
                      % TF = 5 +Z, Y vert
                      % TF = 6 -Z, Y vert
                      % TF = 7 ISO
plot_tor(5).el_type = [2 4]; % plotting elements other than 5 
