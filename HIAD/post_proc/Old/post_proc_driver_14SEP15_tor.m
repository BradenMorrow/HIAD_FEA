...
plot_tor(1).el_type = [4]; % plotting elements other than 5 
...
plot_tor(2).LS = LS; % load step plotted
...
plot_tor(3).n = 15; % Number of slats on cross section, 0 for o plot
plot_tor(3).LS = LS; % load step plotted
...
plot_tor(3).color = [.5 .5 .5]; % color  of fill or wireframe
plot_tor(3).view = 6; % TF = 1 for Bottom View, 
    % TF = 2 for Top View
    % TF = 3 for Right View
    % TF = 4 for Left View
    % TF = 5 for Back View
...
plot_tor(4).LS = LS; % load step plotted
...
f = 5;
plot_tor(f).n = 15; % Number of slats on cross section, 0 for no plot
plot_tor(f).LS = LS; % load step plotted
plot_tor(f).PF = 0; % plot forces?
plot_tor(f).plot_type = 5; % 1 = DCP, 2 = DF, 3 = UF, 4 = DWF, 5 = UWF 
plot_tor(f).def=0;
plot_tor(f).fig = 2; %figure number
plot_tor(f).u_comp = 4; % U_component = 1 for x, 2 for y, 3 for z, 4 for magnitude
plot_tor(f).auto_scale = 0; % auto color scale switch 
plot_tor(f).caxis = [0,5]; % Min and max displacement scale
plot_tor(f).color = 'k'; % color  of fill or wireframe
plot_tor(f).view = 6; % TF = 1 for Bottom View, 
    % TF = 2 for Top View
    % TF = 3 for Right View
    % TF = 4 for Left View
    % TF = 5 for Back View
    % TF = 6 for Front View
    % TF = 7 for Isometric View
plot_tor(f).el_type = []; % plotting elements other than 5 
plot_tor(f).scale = 1; % scale of deformation
plot_tor(f).triad = 1; % triad switch
plot_tor(f).triad_loc = [0 0 0]'; % triad location
plot_tor(f).axis = 0; % axis switch
