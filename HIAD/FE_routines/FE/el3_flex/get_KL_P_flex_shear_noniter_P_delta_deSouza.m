...
k = el_in.flex.k;
ke = el_in.flex.ke;
v_kappa = L^2*el_in.flex.v_kappa;
v_gamma = L*el_in.flex.v_gamma;

l_tilda = zeros(n*2,n*5);
l_tilda(1:2:end,2:5:end) = v_kappa;
l_tilda(1:2:end,4:5:end) = v_gamma;
l_tilda(2:2:end,2:5:end) = -v_kappa;
l_tilda(2:2:end,4:5:end) = -v_gamma;
...
% Initialize
b = zeros(5*n,5); % Force interpolation functions
% dD = zeros(5*n,1); % Section force increment
...
% Loop for iterative form of state determination process
Q = Q + dQ;
...
for i = 1:n
    ...
    % Force interpolation functions at sections
    b((i - 1)*5 + 1:i*5,:) = ...
        [1 0 0 0 0 % [axial Mz My Vz Vy]'
        v((i - 1)*2 + 1) (x/L - 1) (x/L) 0 0
        -v(i*2) 0 0 (x/L - 1) (x/L)
        0 -1/L -1/L 0 0
        0 0 0 1/L 1/L];
end
...
for i = 1:n
    ...
    % Tangent stiffness
    ki((i - 1)*n + 1:(i - 1)*n + 5,(i - 1)*n + 1:(i - 1)*n + 5) = diag([EL*A EL*I EL*I (GLH*A/2 + p*pi*R^2) (GLH*A/2 + p*pi*R^2)]');
    ki((i - 1)*n + 1:(i - 1)*n + 3,(i - 1)*n + 1:(i - 1)*n + 3) = ki((i - 1)*n + 1:(i - 1)*n + 3,(i - 1)*n + 1:(i - 1)*n + 3) + l(:,:,i)'*diag(EA(:,i))*l(:,:,i);
    
    % Reordered DOFs [axial Mz My Vz Vy]'
    Dr(:,i) = [sum(sig(:,i))  + p*pi*R^2*2*cotd(beta)^2 + EL*A*d((i - 1)*n + 1) - p*pi*R^2
        sum(sig(:,i).*l(:,2,i))
        sum(sig(:,i).*l(:,3,i))
        0
        0] + diag([EL*A EL*I EL*I (GLH*A/2 + p*pi*R^2) (GLH*A/2 + p*pi*R^2)]')*d((i - 1)*n + 1:(i - 1)*n + 5).*[0 1 1 1 1]'; % Includes axial prestress and shell forces
end

k = ki;
Dr = Dr(:);

% v = [(v_kappa*d(2:5:end) + v_gamma*d(4:5:end))';
%     (-v_kappa*d(3:5:end) - v_gamma*d(5:5:end))'];
% v = v(:);
v = l_tilda*d;
...
for i = 1:n
    ...
    b_star((i - 1)*5 + 1:i*5,:) = ...
        [1 0 0 0 0 % [axial Mz My Vz Vy]'
        v((i - 1)*2 + 1)/2 (x/L - 1) (x/L) 0 0
        -v(i*2)/2 0 0 (x/L - 1) (x/L)
        0 -1/L -1/L 0 0
        0 0 0 1/L 1/L];
end
...
q_star = zeros(5,1);
for i = 1:n
    q_star = q_star + L*b_star((i - 1)*5 + 1:i*5,:)*d((i - 1)*5 + 1:i*5)*wi(i);
end
...
for i = 1:n
    ...
    % Equilibrium section stiffness matrix
    ke = 0;
