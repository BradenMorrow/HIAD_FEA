...
if (undef)
    ...
    line(x(ind2,:)',y(ind2,:)',z(ind2,:)','color','k','linestyle','-','linewidth',2) % ,'marker','o') % 
    line(x(ind3,:)',y(ind3,:)',z(ind3,:)','color','k','linestyle','-','linewidth',5) % ,'marker','o') % 
    line(x(ind4,:)',y(ind4,:)',z(ind4,:)','color','c','linestyle','-','linewidth',2)
end
...
if (def)
    if sum(U) ~=0
        ...
        line(x1(ind3,:)',y1(ind3,:)',z1(ind3,:)','color','b','linestyle','-','linewidth',1) % ,'marker','o' % 
...
F_scale(F_scale == 0) = [];
scale_F = .5; % .15*max(max(abs(nodes)))/max(F_scale);
