...
EH = el_in.propsLH(2); % Hoop modulus
GLH = el_in.propsLH(3); % Inplane shear modulus
nuHL = el_in.propsLH(4); % Poisson's ration, nu_HL
nuLH = nuHL/EH*EL; % Poisson's ration, nu_LH
...
while conv ~= EC && j < max_iter
    ...
    for i = 1:n
        ...
        Fnu = p*pi*R^2*(1 - 2*nuLH);
        
        %%%%%
        a1 = -Fnu/d(2,i); % -Fnu*dyde; % 
        a1(isnan(a1) || isinf(a1)) = 0;
        
        a2 = Fnu*d(1,i)/d(2,i)^2; % Fnu*dydk; % 
...
%         k_dv(2,2,i) = a2;
        ...
        Dr(:,i) = [sum(sig(:,i))  + p*pi*R^2*2*cotd(beta)^2 + EL*A*d(1,i) - p*pi*R^2
            sum(sig(:,i).*l(:,2,i)) + 1000 % - Fnu*d(1,i)/d(2,i) %(R - y_bar)
            sum(sig(:,i).*l(:,3,i))
            0
...
% % % Kp = zeros(size(K));
% % % for i = 1:n
% % %     Kp = Kp - p*pi*R^2*L/2*wi(i)*(f(:,:,i).*b(:,:,i))*K;
% % % end
% % % K = K + Kp;
