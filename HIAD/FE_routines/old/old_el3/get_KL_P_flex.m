...
for i = 1:n
    l(:,:,i) = [ones(size(alpha)) -R*cosd(alpha) R*sind(alpha)]; % Location of fibers (cartesian)
end
...
while conv ~= EC && j < max_iter
    ...
    for i = 1:n
        ...
        k_shell = diag([EL*A EL*I EL*I]'); % Including elastic shell
        k(:,:,i) = k(:,:,i) + k_shell;
        ...
        %     sum(sig(:,i).*l(:,2,i))
        %     sum(sig(:,i))];
        % Reordered DOFs [axial Mz My]'
        Dr(:,i) = [sum(sig(:,i))  + p*pi*R^2*2*cotd(beta)^2 + EL*A*d(1,i) - p*pi*R^2
            sum(sig(:,i).*l(:,2,i))
            sum(sig(:,i).*l(:,3,i))] + k_shell*d(:,i).*[0 1 1]'; % includes axial prestress and shell forces
    ...
    % Absolute criteria
    tol_a = [tol*ones(1,n); tol*L*ones(2,n)];
...
if j == max_iter && conv ~= EC
    warning('Max iter reached, get_KL_P_flex')
    el_out.break = 1;
else
    el_out.break = 0;
end
