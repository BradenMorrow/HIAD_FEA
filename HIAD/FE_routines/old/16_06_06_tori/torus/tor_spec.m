function [tor_nodes1,U1] = tor_spec(theta,I_theta_cable)
...
tor_nodes_end = tor_nodes(:,:,329);
U1 = [tor_nodes_end - tor_nodes1 zeros(size(tor_nodes1))];

% % % uind = true(size(U1,1),1);
% % % uind(I_theta_cable) = 0;
% % % U1(uind,:) = 0;
U1 = U1';
U1 = U1(:);
...
% % % box on
% % % hold on
% % % plot(theta*180/pi,hypot(tor_nodes1(:,1),tor_nodes(:,2)),'b-')
% % % plot(theta*180/pi,hypot(tor_nodes_end(:,1),tor_nodes_end(:,2)),'g-')
% % % 
% % % 
