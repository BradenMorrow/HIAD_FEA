function [d] = cord_model_27MAY15(eps,F,load_point,unload_point,d_LOAD,d_UNLOAD,eps_rate)
%CORD_MODEL_06MAY15
...
d_orig = d_LOAD; % d_LOAD(d_LOAD(:,2) > max([unload_point(:,2); eps]),:);
...
for i = 1:size(load_point,1)
    ...
    d_unload = [d_unload(d_unload(:,2) < eps2(1) & d_unload(:,1) < f2(1),:); f2 eps2];
end
...
if size(d_unload,1) == 1 || isempty(d_unload)
    d_unload = d_LOAD;
end
...
if eps - 1e-6 < 0
    ...
    if eps_rate > 0
        if ~isempty(d_load)
            d0 = [0 0; d_load; d_orig(d_orig(:,2) > d_load(end,2),:)];
        else
            d0 = d_orig;
        end
...
if isnan(d(1,1)) || isnan(d(2,1)) || isnan(d(1,2)) || isnan(d(2,2))
    a = 1;
end
...
% plot(d0(:,2),d0(:,1),'bx-')
% plot(d(:,2),d(:,1),'r-')
% plot(d_unload(:,2),d_unload(:,1),'g--')
% plot(d_load(:,2),d_load(:,1),'g--')
% plot(d_orig(d_orig(:,2) > d_load(end,2),2),d_orig(d_orig(:,2) > d_load(end,2),1),'g--')
% 
% plot(eps,f0,'go')
% 
% test = load('cord1.txt');
% plot(test(:,1)/100,test(:,2),'c-')
% 
% xlim([0 .03])
% ylim([-50 3000])
