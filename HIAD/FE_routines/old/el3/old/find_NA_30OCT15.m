function [eps_cord,f_cord,k_cord,phi_i,phi_j,kappa2,y_bar,iter,d1i,d1j,PV_work,kappa3] = find_NA(D,D0,eps_cord_old,f_cord_old,p,r,alpha,EL,EH,GLH,nuHL,t,EI0,y_bar0,L,cords,interp_meth,el,beta,P0)
...
% Find location of NA
while norm(f) > tol0 && iter <= max_iter && sum(f_prime) ~= 0
    % ----------
...
%     f0 = f;
%     f = sum(f_cord)' + p*pi*r^2*2*cotd(beta)^2 + 2*pi*r*t*EL*eps_ax - 2*pi*r*t*EL*y_bar.*kappa2 - p*pi*r^2 - sum(f_cord - f_cord_old)' - 2*pi*r*t*EL*eps_ax;
    f = sum(f_cord - f_cord_old)' + 2*pi*r*t*EL*eps_ax + 2*pi*r*t*EL*y_bar.*kappa2 - P0(1) - EA'*d_eps_ax;
    ...
    [eps_cord_plus] = get_eps(eps_cord_old + eps_ax,y_bar_plus,r,phi_i,phi_j,alpha,kappa2);
%     eps_cord_plus = eps_cord_old + delta_eps_cord_plus;
    ...
    for i = 1:2
        for j = 1:length(alpha)
            ...
            EA_plus = sum(k_cord_plus) + 2*pi*r*t*EL;
         end
    end
...
%     f_plus = sum(f_cord_plus)' + p*pi*r^2*2*cotd(beta)^2 + 2*pi*r*t*EL*eps_ax - 2*pi*r*t*EL*y_bar_plus.*kappa2 - p*pi*r^2 - sum(f_cord_plus - f_cord_old)' - 2*pi*r*t*EL*eps_ax;
    f_plus = sum(f_cord_plus - f_cord_old)' + 2*pi*r*t*EL*eps_ax + 2*pi*r*t*EL*y_bar_plus.*kappa2 - P0(1) - EA_plus'*d_eps_ax;
